...
parser = argparse.ArgumentPaser(description='output feature annotation')
...
anno_text = pd.read_csv(args.txt, header=None, sep='\t')
# read action label directory
action_label = pd.reac_csv(args.label, header=None, sep='\t')
...
img_list = ps.Series(os.listdir(args.imlist).str.extract('(.+)_(.+)\.(.+)', expand=True))
# sort
...
pd_label = pd.Serises(frame_label, name="Label")
...
for i, num in enumerate(feature_labels["Frame"]):
    labeled_imglist.append("img_{0:05d}.png".format(num))
...
# output text file 
df_labels.to_csv(args.outdir + '/feature_images_{}.txt'.format())