...
def select_region_mask(image):
    rows, cols, ch = image.shape
    bottom_left   = [cols*0.15, rows*1]
    top_left      = [cols*0.3, rows*0.22]
    bottom_right = [cols*0.85, rows*1]
    top_right     = [cols*0.7, rows*0.22]
    # mask is an array of polygons and data type must be integer
    region = np.array([[bottom_left, top_left, top_right, bottom_right]], dtype=np.int32)
    # create the mask using 
    mask = np.zeros_like(image)
    if len(image.shape)==3:
        cv2.fillPoly(mask, region, (255,)*mask.shape[2])
    else:
        cv2.fillPoly(mask, region, 255)

    return cv2.bitwise_and(image, mask)

def plot_player_position(image,person_pose_array):
    # person_1
    rightfoot_1 = np.delete(person_pose_array[0,10],2)
    leftfoot_1 = np.delete(person_pose_array[0,13],2)
    # person_2
    rightfoot_2 = np.delete(person_pose_array[1,10],2)
    leftfoot_2 = np.delete(person_pose_array[1,13],2)

    # identify top and bottom player and calculate the position
    if rightfoot_1[1] < rightfoot_2[1]:
        top_foot = (rightfoot_1+leftfoot_1)/2
        bottom_foot = (rightfoot_2+leftfoot_2)/2
    else:
        top_foot = (rightfoot_2+leftfoot_2)/2
        bottom_foot = (rightfoot_1+leftfoot_1)/2
    # convert numpy array(int) to tuple
    top_foot = tuple(np.array(top_foot, dtype=int))
    bottom_foot = tuple(np.array(bottom_foot, dtype=int)) 
    # polt player position with white circle
    cv2.circle(image, top_foot, 4, (0,0,255), -1) 
    cv2.circle(image, bottom_foot, 4, (0,0,255), -1)
...
def perspective_transform(image):
    # position of corner in the court
    pre_posi = np.float32([[410,270],[870,270],[230,695],[1040,695]])
    # plot position
    tar_posi = np.float32([[0,0],[610,0],[0,670],[610,670]])
    M = cv2.getPerspectiveTransform(pre_posi,tar_posi)
    court = cv2.warpPerspective(image,M,(610,670))
    return court
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder', '-f', default=None, help='set images path for video') # set images for video path
    ...
    #set path to images
    path_dir = '/home/daichi/Share_with_Windows/images/badminton_1/'
    # read opencourt image
    bg = cv2.imread('./opencourt.png')
    # Perspective Transform
    ptf_img = perspective_transform(bg) 

    ## create video
    fourcc = cv2.VideoWriter_fourcc(*'MPEG')
    video = cv2.VideoWriter('video.mp4', fourcc, 20.0, (1280,720))
    video2 = cv2.VideoWriter('video2.mp4', fourcc, 20.0, (610,670))

    for i in range(1,100):
        # read image
        img = cv2.imread(path_dir+"VictorKoreaOpen2016Badminton_1{0:07d}.png".format(i))
        # select region
        #dst = select_region_mask(img)
        ...
        # detect and plot the position of each player
        bg = plot_player_position(bg, person_pose_array)
        # Perspective Transform
        ptf_img = perspective_transform(bg) 
        video2.write(ptf_img)
...
# plot postion trajectory to 
    cv2.imwrite('point_summary.png',ptf_img)
